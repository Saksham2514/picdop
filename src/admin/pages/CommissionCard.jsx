...
const CommissionCard = ({title,value,setBreakpoint,ind,setData,setError}) => {
    ...
    const handleUpdate = ()=>{
        axios
        .put(`${process.env.REACT_APP_BACKEND_URL}updateComission`, updateObject)
        .then((res) => { 
        if (res?.data) {
            setUpdate(true)
            setData(res.data);
        } else 
            setError("Couldn't Update Commission");
        })
